import { defineAnimation } from 'react-native-reanimated/src/reanimated2/animation/util';
import type {
  Animation,
  AnimationCallback,
  AnimatableValue,
...
export function withSpringTimestamp<T extends AnimatableValue>(
  toValue: AnimatableValue,
  timestamp: number,
  ...
  return defineAnimation<SpringAnimation>(toValue, () => {
    ...
    function onStart(
      ...
      if (previousAnimation) {
        ...
        animation.lastTimestamp = previousAnimation.lastTimestamp || timestamp;
      } else {
        console.log(3333, now, timestamp);
