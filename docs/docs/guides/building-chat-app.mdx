...
<ComparisonTable
  left={
    <Image src="/img/keyboard-chat-translucent.png" width={65} />
  }
  right={
    <Image src="/img/keyboard-chat-non-translucent.png" width={65} />
  }
  leftText={
    <i>
      By default keyboard is transparent, messages behind the keyboard is visible (Messages app style)
    </i>
  }
  rightText={
    <i>
      Non-transparent keyboard, messages behind the keyboard are hidden (Instagram app style)
    </i>
  }
/>

You can reproduce this with [`KeyboardEffects`](../api/components/keyboard-effects). Just render an opaque view — colored to match your chat background — behind it. The keyboard then blends with your color instead of the system material:
