...
## Matching the keyboard background (Instagram-style)

By default the iOS keyboard is translucent — it blurs whatever sits behind it. Many chat apps (Instagram, and dark-themed messengers in general) instead make the keyboard appear to sit on a **solid background that matches the conversation**, so the keys blend into the chat rather than showing the system blur.

You can reproduce this with [`KeyboardEffects`](../api/components/keyboard-effects). Make the keyboard `translucent` and render an opaque view — colored to match your chat background — behind it. The keyboard then blends with your color instead of the system material:
...
Because `KeyboardEffects` is built on `KeyboardStickyView`, the backdrop tracks the keyboard as it opens and closes, so the solid background stays aligned with the keys throughout the animation. Swap the plain `View` for a gradient or an animated Skia canvas and you get richer effects (an Apple Intelligence / Siri-style glow) — but a single opaque color is all you need for the Instagram look.

:::info iOS only
The translucent keyboard is an iOS capability, so this technique only affects iOS. On Android the keyboard is opaque and the backdrop won't show through — the rest of your chat layout keeps working unchanged. On iOS 26+ the keyboard's rounded corners are matched automatically.
:::
