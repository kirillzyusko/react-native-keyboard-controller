...
<ComparisonTable
  ...
  leftText={
    <i>
      By default keyboard is transparent, messages behind the keyboard are
...
You can achieve this with [`KeyboardEffects`](../api/components/keyboard-effects). Just render an opaque view — colored to match your chat background — behind the keyboard. The keyboard then blends with your color instead of the system material:
...
Because `KeyboardEffects` is built on `KeyboardStickyView`, the backdrop tracks the keyboard as it opens and closes, so the solid background stays aligned with the keys throughout the animation. Swap the plain `View` for a gradient or an animated Skia canvas and you get richer effects (an Apple Intelligence / Siri-style glow).
