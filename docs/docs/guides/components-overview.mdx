...
You can control how it adjusts using the `behavior` prop (`padding`, `height`, `position`, `translate-with-padding`). Navigation headers and modals are handled automatically — use `keyboardVerticalOffset` only if you need extra space above the keyboard.
