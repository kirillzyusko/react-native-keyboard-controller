...
`KeyboardEffects` is, in a sense, the **opposite** of `KeyboardBackgroundView`. Rather than pulling the keyboard's background _into_ your UI to match it, it lets you render arbitrary content — colors, gradients, images, animations — _behind_ the keyboard. By default, the iOS keyboard already has its own translucent blur, so your app's content is visible behind it, but blurred and tinted by the system. Setting `translucent` on `KeyboardEffects` removes that native blur/backdrop entirely, so whatever you render behind it shows through crisp and unaltered, in its place.

That single mechanism covers both ends of the spectrum: render a solid, opaque color that matches your chat background and you get a fully non-transparent keyboard (à la Instagram); render an animated gradient or Skia shader and you get effects like the Apple Intelligence / Siri keyboard glow.
...
| `KeyboardEffects`         | Renders any content behind the keyboard    | ❌ (visual only)           | Themed/animated keyboard, solid backgrounds | Removes native blur, replaces what shows _through_ the keyboard (iOS) |
