...
Extra offset (in points) to add above the keyboard. Default is `0`.

The view’s position on screen is measured automatically using `measureInWindow`, so navigation headers, modals, and other layout offsets are already accounted for. You do **not** need to pass `useHeaderHeight()` or manually compensate for navigation bars — this is handled internally.

Use `keyboardVerticalOffset` only when you want to add **extra** space above the keyboard beyond the automatically calculated position (e.g., to leave room for a toolbar or additional padding).
