...
When `true`, freezes the view at its current position and stops reacting to keyboard changes. Useful when dismissing the keyboard should keep the sticky view in its current position. Default is `false`.
