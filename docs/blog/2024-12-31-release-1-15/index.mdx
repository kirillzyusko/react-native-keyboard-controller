...
Now I fixed that problem and `KeyboardToolbar` now accepts the same properties that `KeyboardStickyView` accepts making a better interoperability between these two components.
