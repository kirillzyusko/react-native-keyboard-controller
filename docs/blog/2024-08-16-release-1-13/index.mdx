...
With this release you can now prevent default actions from happening by calling `e.preventDefault()` in `onNextCallback`, `onPrevCallback`, and `onDoneCallback` props. I think it's a great addition to the existing functionality, because sometimes you may need to have this functionality - for example if you have a custom input between two `TextInput`s (let's say calendar picker) and when you press next button (while you have a focus on first input) most likely you want to open a calendar picker and not skip an input field (which is the default behavior). So now you can call `e.preventDefault()` conditionally and perform your own action to achieve your custom requirements!
