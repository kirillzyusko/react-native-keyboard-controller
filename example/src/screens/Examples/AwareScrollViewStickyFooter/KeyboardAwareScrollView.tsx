...
const KeyboardAwareScrollView: FC<
  ScrollViewProps & {
    /**
     * Adds an extra offset between the keyboard and the focused input.
     * @default 0
     */
    bottomOffset?: number;
  }
> = ({ children, bottomOffset: rawBottomOffset = 0, ...rest }) => {
  ...
  const bottomOffset = useSharedValue(rawBottomOffset);

  // Fix staled values
  useEffect(() => {
    bottomOffset.value = rawBottomOffset;
  ...
  const maybeScroll = useWorkletCallback((e: number, animated = false) => {
    ...
    if (visibleRect - point <= bottomOffset.value) {
      const interpolatedScrollTo = interpolate(
        e,
        [initialKeyboardSize.value, keyboardHeight.value],
        [0, keyboardHeight.value - (height - point) + bottomOffset.value]
      );
      const targetScrollY =
